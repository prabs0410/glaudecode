// Shell-pane session inference (V6 paneId ≠ sessionId). When the active pane is a plain *shell* in
// which the user ran `claude` by hand, GlaudeCode can't know that session's id directly, so it infers
// it from the cwd's sessions. The naive "newest wins, sticky" heuristic could silently lock onto the
// WRONG session when two are live in one repo (audit #11) — a background agent touching session B
// would steal the dock from the foreground session A. This pure fn adds a single-live / margin
// tiebreaker and an explicit `ambiguous` signal so a genuine two-live tie is SURFACED, never guessed.
//
// Pure + injected `now` so it's unit-tested here (test/sessionInference.test.ts) and MIRRORED verbatim
// in desktop/src/sessionInference.ts (the WebView can't import the engine — it pulls the Node-only SDK).
