...
// configHome → a temp dir so the persisted signing key / VAPID key / roster never touch ~/.glaudecode.
const withServer = async (fn: (s: ReturnType<typeof startEngineServer>) => Promise<void>): Promise<void> => {
  const home = mkdtempSync(join(tmpdir(), "gc-icon-"));
  const s = startEngineServer({ token: "icon", configHome: home });
  try {
    await fn(s);
  } finally {
    s.stop();
