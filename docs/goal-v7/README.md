...
- 🛠️ **GATE-FIX** — discovered while running D5: the B4 `bun run verify` script's `bun --cwd <dir> run <script>` form is a **silent no-op** on Bun 1.2.18 (prints `bun run` help, exits 0) — only `cargo check` actually ran. The gate was reporting **false green**. Fixed next; the canonical per-package commands remain authoritative.
